...
public interface ZhuatuParser {
	 ...
	 * 初始化
	 */
	void init(List<Service> serviceList,ZhuatuConfig config);
	/**
	 * 解析页面之前扩展
	 * @param service
	 * @param info
	 ...
	 */
	boolean beforPageProjet(Service service,TuInfo info,int index);
	/**
	 ...
	 *            解析出的项目（列表中的一个）
	 */
	void afterPageProjet(Service service,TuInfo info);
	/**
	 * 对当前页解析出的项目操作
	 * 
	 * @param service
	 ...
	 */
	boolean doReturnProject(Service service, TuInfo info);
	 ...
	 */
	void beforNextService(TuInfo info, Service nextService,int nextIndex);
